        ...
        stage('Determine Event Type') {
            steps {
                script {
                    ...
                    if (!allowedBranches.contains(BRANCH_NAME)) {
                        error("❌ Branch ${BRANCH_NAME} is not allowed. Skipping build.")
                    }
                }
            }
        }
                ...
                script {
                    try {
                        sh '''
                            ...
                            pytest tests/test_calculator_logic.py | tee result.txt
                        '''
                    } catch (err) {
                        def msg = sh(script: "tail -n 10 result.txt", returnStdout: true).trim()
                        // This message will show in GitHub PR check status
                        error("❌ Addition Check Failed: ${msg}")
                    }
                }
            }
        }
